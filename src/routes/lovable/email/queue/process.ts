...
const SENDER_DOMAIN = 'notify.cremedelacremenails.com'
const FROM_DOMAIN = 'notify.cremedelacremenails.com'
...
async function ensureUnsubscribeToken(supabase: any, email: string): Promise<string> {
  ...
  if (lookupError) throw new Error(`Failed to look up unsubscribe token: ${lookupError.message}`)
  if (existingToken?.token && !existingToken.used_at) return existingToken.token
  if (existingToken?.used_at) throw new Error(`Recipient is unsubscribed: ${normalizedEmail}`)
...
async function hydrateLegacyTemplatePayload(
  supabase: any,
  payload: Record<string, any>
): Promise<Record<string, any>> {
  if (payload.html && payload.to && payload.subject) return payload

  const templateName = payload.template_name
  const recipient = payload.recipient_email
  const templateData = payload.template_data ?? {}
  const template = typeof templateName === 'string' ? TEMPLATES[templateName] : null

  if (!template || !recipient) {
    throw new Error(`Queued email missing rendered content and cannot be hydrated: ${templateName ?? 'unknown template'}`)
  }
  ...
  return {
    ...
    label: payload.label ?? templateName,
    idempotency_key: payload.idempotency_key ?? payload.message_id,
    unsubscribe_token: payload.unsubscribe_token ?? await ensureUnsubscribeToken(supabase, recipient),
      ...
      POST: async ({ request }) => {
        ...
        for (const queue of ['auth_emails', 'transactional_emails']) {
          ...
          for (let i = 0; i < messages.length; i++) {
            ...
            let payload = msg.message as Record<string, any>
            ...
            try {
              payload = await hydrateLegacyTemplatePayload(supabase, payload)
